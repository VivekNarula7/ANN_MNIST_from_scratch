...
class NeuralNetwork:
    def __init__(self, layers):
        ...
        for i in range(1, len(layers)):
            limit = np.sqrt(6 / (layers[i-1] + layers[i]))  # Xavier init limit for uniform distribution
...
if __name__ == '__main__':
    ...
    train_losses_1, test_losses_1, train_accuracies_1, test_accuracies_1 = nn1.train(
        X_train, Y_train_onehot, X_test, Y_test, epochs=201, learning_rate=0.1, save_path='models/best_model_1_hidden_sigmoid.pkl'
    )

    # Load the best weights before prediction
    nn1.load_weights('models/best_model_1_hidden_sigmoid.pkl')
    ...
    train_losses_2, test_losses_2, train_accuracies_2, test_accuracies_2 = nn2.train(
        X_train, Y_train_onehot, X_test, Y_test, epochs=201, learning_rate=0.1, save_path='models/best_model_3_hidden_sigmoid.pkl'
    )

    # Load the best weights before prediction
    nn2.load_weights('models/best_model_3_hidden_sigmoid.pkl')
    ...
    plot_metrics(train_losses_2, test_losses_2, train_accuracies_2, test_accuracies_2, filename='nn_3_hidden_layers_metrics_sigmoid')
